...
<table border=3 class="table table-bordered table-striped table-novel">
        ...
        <tbody>
            ...
            <tr style="font-size: x-large;"><td>{{$result->name}}</td>
            <td>{{$result->email}}</td>
            <td>{{$result->phone}}</td>
            <td>{{$result->status}}</td>

            <td>    <a href="{{url('admin_view_buyer_details',$result->id)}}" class="btn btn-success"style="font-size: x-large;">Details</a>
</td>
</tr>
            
@endforeach
            <?php }
        else{ ?><tr>"No records"</tr><?php } ?>
    </tbody>
</table>
</div>
</body>
